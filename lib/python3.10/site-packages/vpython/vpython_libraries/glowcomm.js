...
// test.py calls onmessage, which responds through domessage, using send
// Both test.py and this file are kept alive by sending messages back and forth.
		...
		for (i=0; i<c.length; i++) { // step through the encoded attributes and methods
			...
			if (vecattrs.indexOf(attr) > -1) {
				...
				if (attr == 'choices') {          // menu choices are wrapped in a list
                    ...
                    // '\n' doesn't survive JSON transmission, so in test.py we replace '\n' with '<br>'
...
function fix_location(cfgx) {
    ...
    if ('location' in cfgx) {
        ...
        var id = loc[0]
        if (id == -1) {
            cfgx['pos'] = print_anchor // this doesn't work; throw an error in test.py
        } else {
            var cvs = glowObjs[id]
            var where = loc[1]
